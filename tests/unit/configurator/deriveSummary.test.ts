...
describe('deriveSummary', () => {
  it('matches the brief\'s own reference example for the default state', () => {
    const summary = deriveSummary(DEFAULT_CONFIGURATOR_STATE);
  ...
  it('recalculates area as width × length whenever a dimension changes', () => {
    const summary = deriveSummary(
      withState({ dimensions: { width: 30, length: 50, height: 8 } }),
    );
  ...
  it('rounds a fractional area to the nearest whole square metre', () => {
    const summary = deriveSummary(
      withState({ dimensions: { width: 11, length: 33.5, height: 4 } }),
    );
  ...
  it('formats a half-metre height with one decimal, whole metres without', () => {
    const summary = deriveSummary(
      withState({ dimensions: { width: 24, length: 60, height: 8.5 } }),
    );
  ...
  it.each([
    ...
    expect(deriveSummary(withState({ envelope })).envelopeLabel).toBe(label);
  });

  it('lists scope items in a fixed reading order regardless of toggle order', () => {
    const summary = deriveSummary(withState({ scope: ['roof', 'foundation', 'walls'] }));
  ...
  it('says so plainly when no scope item is selected, instead of an empty string', () => {
    const summary = deriveSummary(withState({ scope: [] }));
  ...
  it.each([
    ...
    expect(deriveSummary(withState({ gates })).gatesLabel).toBe(label);
  });
});
